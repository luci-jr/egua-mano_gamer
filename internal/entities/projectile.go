...
func (pm *ProjectileManager) Update(screenWidth float64) {
	// 1. Atualizar Projéteis (limpos, sem rastro de fagulhas)
...
func (pm *ProjectileManager) Draw(screen *ebiten.Image, ticks int) {
	...
	// 2. Projéteis
	// Açaí em roxo bem mais escuro, homogêneo e puro (estilo açaí paraense legítimo)
	cAcaiOuter := color.RGBA{R: 16, G: 4, B: 20, A: 255}       // Casca do açaí preto-arroxeado profundo
	cAcaiInner := color.RGBA{R: 36, G: 8, B: 44, A: 255}       // Polpa de açaí roxo bem escuro
	cAcaiCore := color.RGBA{R: 48, G: 12, B: 58, A: 255}       // Centro roxo escuro (zero amarelo ou reflexo claro)

	// Rugido Sônico da Onça: onda de choque de pressão (carmesim / laranja / branco sônico, sem bolinhas amarelas)
	cRoarOuter := color.RGBA{R: 200, G: 65, B: 30, A: 220}     // Onda de choque carmesim
	cRoarInner := color.RGBA{R: 240, G: 110, B: 40, A: 240}    // Onda terracota
	cRoarCore := color.RGBA{R: 255, G: 255, B: 255, A: 255}    // Núcleo branco sônico de ar
	...
	for _, p := range pm.Projectiles {
		...
		if p.Kind == ProjKindRoar {
			// Onda de Choque Sônica (Rugido da Onça) - Arcos concêntricos em carmesim e ar
			px := p.X
			py := p.Y
			isUp := p.VY < 0

			if p.IsSpecial {
				// Mega Rugido Alfa: Onda tripla gigante sem amarelo
				cSpecialOrange := color.RGBA{R: 245, G: 115, B: 35, A: 255}
				cSpecialCrimson := color.RGBA{R: 215, G: 50, B: 30, A: 240}
				if isUp {
					ebitenutil.DrawRect(screen, px-12, py+4, 24, 3, cSpecialCrimson)
					ebitenutil.DrawRect(screen, px-10, py+1, 20, 3, cSpecialOrange)
					...
					ebitenutil.DrawRect(screen, px+dir*3, py-7, 3, 14, cSpecialOrange)
		...
		if p.IsSpecial {
			// Super Bolinha de Açaí Gigante Energizada (10x10 roxo bem escuro puro)
			px := p.X - 5.0
			py := p.Y - 5.0
			cAura := color.RGBA{R: 42, G: 10, B: 52, A: 140}
			cAcaiSpecialBody := color.RGBA{R: 34, G: 8, B: 42, A: 255}
			cAcaiSpecialGleam := color.RGBA{R: 48, G: 12, B: 58, A: 255}
		...
		// Bolinhas de Açaí Tradicionais do Garoto (roxo bem escuro autêntico)
		px := p.X - 3.0
		py := p.Y - 3.0

		// Esfera 6x6 pixel art com cantos recortados em roxo bem escuro
		...
		ebitenutil.DrawRect(screen, px+2, py+2, 2, 2, cAcaiCore)
	}
